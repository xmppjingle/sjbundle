 ...
 */
package javax.sdp.parser;

import gov.nist.core.ParserCore;

import javax.sdp.SessionDescriptionImpl;
import javax.sdp.fields.SDPField;
...
public class SDPAnnounceParser extends ParserCore {
    ...
    protected List sdpMessage;

    /**
     * Creates new SDPAnnounceParser
     *
     * @param sdpMessage List of messages to parse.
     */
    public SDPAnnounceParser(List sdpMessage) {
    ...
    public SessionDescriptionImpl parse() throws ParseException {
        ...
        for (Object aSdpMessage : sdpMessage) {
            String field = (String) aSdpMessage;
            try {
                SDPParser sdpParser = ParserFactory.createParser(field);
                SDPField sdpField = sdpParser.parse();
                retval.addField(sdpField);
            } catch (Exception e) {
                // Not Recognized Field. Ignored for now.
            }
        }

        try {
            if (retval.getConnection() != null && retval.getMediaDescriptions(false) != null) {
                return retval;
            }
        } catch (Exception e) {
            // Do Nothing.
        ...
        throw new ParseException("Failed to Parse Necessary Fields.", 0);
