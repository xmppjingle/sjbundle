...
public class PreconditionFields {
    ...
    protected List preconditionAttributes;
     ...
     *
     * @return int size of segmented precondition List of attribute fields
     */
     ...
     * Set Preconditions
     *
     * @param preconditions - List with precondition attributes
     * @throws SdpException -- if precondition attributes is null
     */
    public void setPreconditions(List preconditions) throws SdpException {
    ...
    public void setPreconditionCurr(String status, String directionTag)
        ...
        for (i = 0; i < preconditionAttributes.size(); i++) {
            AttributeField af =
                    (AttributeField) this.preconditionAttributes.get(i);
            ...
            if (af.getValue().contains(status)) {
                if (!af.getValue().contains(directionTag)) {
                    // attribute exists with same status, directionTag updated
                    af.setValue("qos " + status + " " + directionTag);
                    preconditionAttributes.remove(i);
                    preconditionAttributes.add(i, af);
                }
    ...
    public void setPreconditionConfirmStatus(String status, String direction)
        ...
        for (i = 0; i < preconditionAttributes.size(); i++) {
            ...
            // with the value supplied
            if (af.getValue().contains(status)) {
                if (!af.getValue().contains(direction)) {
                    // attribute exists with same status, directionTag updated
     ...
     *
     * @param status - (local, remote, e2e)
     * @return a List with the attribute field that match status-type
     *         (with only one element or null if none exists)
     * @throws SdpParseException
     */
    public List getPreconditionCurr(String status)
        ...
        if (preconditionAttributes == null) {
            ...
            List vCurr = new ArrayList();
            for (Object preconditionAttribute : preconditionAttributes) {
                AttributeField af =
                        (AttributeField) preconditionAttribute;
                ...
                // if it is "curr" attribute, check the status-type
                // and add the attribute value to the List if it
     ...
     *
     * @param status - (local, remote, e2e)
     * @return a List with the attribute fields that match location-tag
     * @throws SdpParseException
     */
    public List getPreconditionDes(String status)
            throws SdpException, SdpParseException {
        if (status == null) {
            throw new SdpException("The status-type is null");
        }
     ...
     * <p>(eg: a=conf:qos remote sendrecv)</p>
     *
     * @return a List with the "conf" attribute fields
     * @throws SdpException
     */
    public List getPreconditionConfirmStatus() throws SdpException {
        if (preconditionAttributes == null) {
