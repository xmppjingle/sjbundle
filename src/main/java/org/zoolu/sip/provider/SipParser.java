    ...
    public NameAddress getNameAddress() {
        ...
        if (begin_url < 0) {
            ...
            if (begin_url < 0) {
                ...
                final int end = indexOf('>');
                setPos(begin_url + 1);
                url = new SipURL(getString(end - begin_url - 1).trim());
...
*/
    ...
    public Message getSipMessage() {  // skip any CRLF sequence
        ...
        int bodyPos = skipChar().getPos(); // skip the LF of the empty line and go the the body

        int end = bodyPos + body_len;
        if (end <= str.length()) {
            index = end;
